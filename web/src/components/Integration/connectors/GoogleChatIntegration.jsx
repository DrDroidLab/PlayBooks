...
const GoogleChatIntegration = () => {
  const [savedGSpaces, setSavedGSpaces] = useState([]);
  const [selectedGSpaces, setSelectedGSpaces] = useState([]);
  const [availableGSpaces, setAvailableGSpaces] = useState([]);
  ...
  const getGoogleSpacesList = API.useGetGoogleSpacesList();
  const registerGoogleSpaces = API.useRegisterGoogleSpaces();
  ...
  useEffect(() => {
    getGoogleSpacesList(
      res => {
        if (res.data.spaces) {
          setAvailableGSpaces(res.data.spaces);
        }
        if (res.data.registered_spaces) {
          setSavedGSpaces(res.data.registered_spaces);
        }
      },
      err => {
        console.error(err);
  ...
  const onGSpaceSelectionChange = (event, selectedOptions, reason) => {
    if (reason === 'selectOption') {
      ...
      if (duplicateId) {
        filtered = selectedOptions.filter(({ name }, index) => name !== duplicateId[0]);
      } else {
  ...
  return (
    <>
      ...
      <div className={styles['container']}>
        ...
        <div className={styles['eventTypeSelectionSection']}>
          <div style={{ fontSize: '13px', width: '150px', margin: 'auto' }}>Available Spaces</div>
          <Autocomplete
            multiple
            disableCloseOnSelect
            options={availableGSpaces}
            getOptionLabel={option => (option.display_name ? option.display_name : option.name)}
            ...
            renderInput={params => (
              <TextField {...params} variant="standard" placeholder="Select Spaces" />
            )}
