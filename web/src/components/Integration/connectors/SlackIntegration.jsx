...
const SlackIntegration = () => {
  ...
  return (
    <>
      <Heading heading="Slack Integration Setup" onTimeRangeChangeCb={false} onRefreshCb={false} />
      <div className={styles.container}>
        <div className={styles.heading}>Enable Auto-RCA recommendation for your channels</div>
        {data?.assets?.[0]?.slack?.assets?.map(asset => (
          ...
          >
            ...
            <Switch onChange={() => handleToggle(asset.id)} checked={!!selected[asset.id]} />
          </div>
        ))}
