...
const ConnectorUpdateOverlay = ({ isOpen, toggleOverlay }) => {
  const [updateConnector, { isLoading, isSuccess, status }] = useUpdateConnectorMutation();
          ...
          <div className={styles['actionOverlay']}>
            <div className="flex items-center justify-between">
              <header className="text-gray-500">
                Update {vpcEnabled ? 'Agent Proxy' : currentConnector?.displayTitle} Keys
              </header>
              <CloseRounded onClick={toggleOverlay} className="text-gray-500 cursor-pointer" />
            </div>
            {(vpcEnabled ? agentProxy?.keyOptions : keyOptions)?.map((option, i) => (
              <div
                key={i}
                style={{ margin: '5px 0' }}
                className={styles['eventTypeSelectionSection']}
              >
                <div className={styles['content']} style={{ fontSize: '12px', marginLeft: '2px' }}>
                  {option.display_name}
                </div>
                <ValueComponent
                  valueType={'STRING'}
