...
class RemoteServetAssetManager(ConnectorAssetManager):
    ...
    def get_asset_model_options(self, model_type: ConnectorMetadataModelType, model_uid_metadata_list):
        if model_type == ConnectorMetadataModelType.SSH_SERVER:
            all_servers = []
    ...
    def get_asset_model_values(self, account: Account, model_type: ConnectorMetadataModelType,
        ...
        if model_type == ConnectorMetadataModelType.SSH_SERVER and (
            ...
            rm_models = rm_models.filter(model_type=ConnectorMetadataModelType.SSH_SERVER)
            if filter_servers:
                rm_models = rm_models.filter(model_uid__in=filter_servers)
        rm_asset_protos = []
        for asset in rm_models:
            if asset.model_type == ConnectorMetadataModelType.SSH_SERVER:
