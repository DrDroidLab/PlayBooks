...
class DatadogAssetManager(ConnectorAssetManager):
    ...
    def get_asset_model_options(self, model_type: ConnectorMetadataModelType, model_uid_metadata_list):
        if model_type == ConnectorMetadataModelType.DATADOG_SERVICE:
    ...
    def get_asset_model_values(self, account: Account, model_type: ConnectorMetadataModelType,
        ...
        if model_type == ConnectorMetadataModelType.DATADOG_SERVICE and (
            ...
            dd_models = dd_models.filter(model_type=ConnectorMetadataModelType.DATADOG_SERVICE)
            if filter_services:
                dd_models = dd_models.filter(model_uid__in=filter_services)
        dd_asset_protos = []
        for asset in dd_models:
            if asset.model_type == ConnectorMetadataModelType.DATADOG_SERVICE:
