...
class NewRelicAssetManager(ConnectorAssetManager):
    ...
    def get_asset_model_options(self, model_type: SourceModelType, model_uid_metadata_list):
        ...
        elif model_type == SourceModelType.NEW_RELIC_NRQL:
            return ConnectorModelTypeOptions(model_type=model_type)
        else:
            return None
    ...
    def get_asset_model_values(self, account: Account, model_type: SourceModelType,
        ...
        if model_type == SourceModelType.NEW_RELIC_ENTITY_APPLICATION and (
            ...
            nr_models = nr_models.filter(model_type=SourceModelType.NEW_RELIC_ENTITY_APPLICATION)
            if filter_applications:
                nr_models = nr_models.filter(metadata__name__in=filter_applications)
        elif model_type == SourceModelType.NEW_RELIC_ENTITY_DASHBOARD and (
            ...
            nr_models = nr_models.filter(model_type=SourceModelType.NEW_RELIC_ENTITY_DASHBOARD)
        ...
        for asset in nr_models:
            if asset.model_type == SourceModelType.NEW_RELIC_ENTITY_APPLICATION:
            ...
            elif asset.model_type == SourceModelType.NEW_RELIC_ENTITY_DASHBOARD:
