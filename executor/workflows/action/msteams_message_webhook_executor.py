...
class MSTeamsMessageWebhookExecutor(WorkflowActionExecutor):
    ...
    def __init__(self):
        ...
        print("reached initiator of class")

    def get_action_connector_processor(self, ms_teams_connector: ConnectorProto, **kwargs):
        if not ms_teams_connector:
            db_connector = get_db_connectors(connector_type=Source.MS_TEAMS)
            if not db_connector:
                raise ValueError('MS Teams connector is not configured')
    ...
    def execute(self, action: WorkflowAction, execution_output: [InterpretationProto],
        ...
        payload = {"type": "message","attachments": [{"contentType": "application/vnd.microsoft.card.adaptive","content": {"type": "AdaptiveCard","version": "1.2","body": blocks,"actions": [{"type": "Action.OpenUrl","title": f"View {playbook_name} Playbook","url": f"{playbook_url}"}]}}]}
        message_params = {'payload':payload}
        try:
            msteams_api_processor = self.get_action_connector_processor(connector)
