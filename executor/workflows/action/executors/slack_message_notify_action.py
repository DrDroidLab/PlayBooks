...
from protos.playbooks.workflow_actions.slack_message_notify_action_pb2 import SlackMessageNotify
...
class SlackMessageExecutor(WorkflowActionExecutor):
    ...
    def execute(self, account: Account, action: WorkflowAction, execution_output: [Interpretation]):
        slack_message_notify: SlackMessageNotify = action.slack_message_notify
        connector_id = slack_message_notify.slack_connector_id.value
        channel_id = slack_message_notify.slack_channel_id.value
        if not connector_id or not channel_id:
            raise ValueError('Slack connector id and slack channel id are required for slack message action')
